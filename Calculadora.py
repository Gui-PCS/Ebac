...
def calculo(calc):                          # resolve uma operação (definida por calc[1]) de 2 números (calc[0] e calc[2])
    if calc[1] == '+':
        result = int(calc[0])+int(calc[2])  # Soma
    elif calc[1] == '-':
        result = int(calc[0])-int(calc[2])  # Subtração
    elif calc[1] == '*':
        result = int(calc[0])*int(calc[2])  # Múltiplicação
    elif calc[1] == '/':
        result = int(int(calc[0])/int(calc[2])) # Divisão
    else:
        result = ("Operação invalida")      # Se um valor invalido for fornecido
    return(result)

def calculadora(op):                        # prepara a operação para ser resolvida na função Calculo
    ops = r"(\+|\-|\*|/)"                   # lista de operadores suportados
    calc = re.split(ops, op)                # transforma a sting op em uma lista de números e operadores
    if len(calc) == 3:                      # Se a lista tem 3 elementos chama a função calculo para retornar o resultado da operação
        return(calculo(calc))
    elif len(calc) > 3:
